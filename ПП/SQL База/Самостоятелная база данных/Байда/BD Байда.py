...
def error(self):
    message = QMessageBox()
    message.setFont(QtGui.QFont("Times", 14))
...
class MyWin(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        conn = sqlite3.connect('Байда.db')
        cursor = conn.cursor()
        cursor.execute('Select * from Perecelenci')
        data = []
        ...
        self.ui.tableWidget.setHorizontalHeaderLabels(['№','Населённый пункт','Область','Национальность','Дата рождения','Фамилия', 'Откуда прибыл'])
    ...
    def error(self):
        ...
        message.setWindowTitle("Ошибка!")
        message.setIcon(QMessageBox.Critical)
        message.setText("Данных не найдено!")
        message.exec_()
    ...
    def on_selected_1(self):
        ...
        if self.ui.radioButton.isChecked() and len(text) !=0:
            try:
                ...
                conn = sqlite3.connect('Байда.db')
                cursor2 = conn.cursor()
                ss = 'Населённый_пункт='+str('"'+text+'"')
                print(ss)
                cursor2.execute('Select * from Perecelenci WHERE '+str(ss))
                proba = []
            ...
            try:
                ...
                conn = sqlite3.connect('Байда.db')
                cursor2 = conn.cursor()
                ss2 = 'Фамилия='+str('"'+text+'"')
                print(ss2)
                cursor2.execute('Select * from Perecelenci WHERE '+str(ss2))
                proba = []
                proba = cursor2.fetchall()
            ...
            try:
                ...
                conn = sqlite3.connect('Байда.db')
                cursor2 = conn.cursor()
                ss3 = 'Откуда_прибыл='+str('"'+text+'"')
                print(ss3)
                cursor2.execute('Select * from Perecelenci WHERE '+str(ss3))
                proba = []
                proba = cursor2.fetchall()
    ...
    def udalit(self):
        try:
            def delete_sqlite_record(dev_id):
                try:
                    sqlite_connection = sqlite3.connect('Байда.db')
                    cursor = sqlite_connection.cursor()
                    print("Подключен к SQLite")

                    sql_update_query = """DELETE from Perecelenci where № = ?"""
            ...
            print("ОШИБКА, выберите поле для удаления!!!")
            message = QMessageBox()
            message.setFont(QtGui.QFont("Times", 14))
            message.setWindowIcon(QtGui.QIcon("Error.png"))
            message.setWindowTitle("Ошибка!")
            message.setIcon(QMessageBox.Critical)
            message.setText("Выберите поле для удаления!")
            message.exec_()
    def obnovit(self):
        try:
            rows = sorted(set(index.row() for index in self.ui.tableWidget.selectedIndexes()))
            ...
            print(text_2)
            text_3 = self.ui.tableWidget.item(row, 5).text()
            if int(col) == 0:
                def update_multiple_records(record_list):
                    try:
                        ...
                        sqlite_update_query = """Update Perecelenci set № = ? where Фамилия = ?"""
                ...
                def update_multiple_records(record_list):
                    try:
                        ...
                        sqlite_update_query = """Update Perecelenci set Населённый_пункт = ? where № = ?"""
                        cursor.executemany(sqlite_update_query, record_list)
                        sqlite_connection.commit()
                        print("Записей", cursor.rowcount, ". Успешно обновлены")
                        sqlite_connection.commit()
                ...
                def update_multiple_records(record_list):
                    try:
                        ...
                        sqlite_update_query = """Update Perecelenci set Область = ? where № = ?"""
                        cursor.executemany(sqlite_update_query, record_list)
                        sqlite_connection.commit()
                        print("Записей", cursor.rowcount, ". Успешно обновлены")
                        sqlite_connection.commit()
                ...
                def update_multiple_records(record_list):
                    try:
                        ...
                        sqlite_update_query = """Update Perecelenci set Национальность = ? where № = ?"""
                        cursor.executemany(sqlite_update_query, record_list)
                        sqlite_connection.commit()
                        print("Записей", cursor.rowcount, ". Успешно обновлены")
                        sqlite_connection.commit()
                ...
                def update_multiple_records(record_list):
                    try:
                        ...
                        sqlite_update_query = """Update Perecelenci set Дата_рождения = ? where № = ?"""
                        cursor.executemany(sqlite_update_query, record_list)
                        sqlite_connection.commit()
                        print("Записей", cursor.rowcount, ". Успешно обновлены")
                        sqlite_connection.commit()
                ...
                def update_multiple_records(record_list):
                    try:
                        ...
                        sqlite_update_query = """Update Perecelenci set Фамилия = ? where № = ?"""
                        cursor.executemany(sqlite_update_query, record_list)
                        sqlite_connection.commit()
                        print("Записей", cursor.rowcount, ". Успешно обновлены")
                        sqlite_connection.commit()
                ...
                def update_multiple_records(record_list):
                    try:
                        ...
                        sqlite_update_query = """Update Perecelenci set Откуда_прибыл = ? where № = ?"""
                        cursor.executemany(sqlite_update_query, record_list)
                        sqlite_connection.commit()
                        print("Записей", cursor.rowcount, ". Успешно обновлены")
                        sqlite_connection.commit()
            ...
            print("ОШИБКА, выберите поле для обновления!!!")
            message = QMessageBox()
            message.setFont(QtGui.QFont("Times", 14))
            message.setWindowIcon(QtGui.QIcon("Error.png"))
            message.setWindowTitle("Ошибка!")
            message.setIcon(QMessageBox.Critical)
            message.setText("Выберите поле для обновления!")
            message.exec_()
...
class Window1(QWidget):
    def __init__(self, parent = None):
        ...
        self.tableWidget_2.setHorizontalHeaderLabels(['№','Населённый пункт','Область','Национальность','Дата рождения','Фамилия', 'Откуда прибыл'])
    ...
    def texty(self):
        ...
        def insert_multiple_records(records):
            try:
                ...
                sqlite_insert_query = """INSERT INTO Perecelenci VALUES (?, ?, ?, ?, ?, ?, ?);"""

                cursor.executemany(sqlite_insert_query, records)
                sqlite_connection.commit()
                print("Записи успешно вставлены в таблицу Perecelenci", cursor.rowcount)
                sqlite_connection.commit()
                cursor.close()
